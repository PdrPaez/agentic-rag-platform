...
The repository is currently synchronized through `ARAG-152`. Recent validated hardening includes deterministic ranking tie-breaking, independent benchmark latency measurement, structured retrieval metadata, recursive trace-secret redaction, controlled provider and demo-seed failures, refreshed evaluation artifacts, provider error correlation, complete runtime configuration documentation, and a robust real HTTP smoke workflow with dynamic port allocation, bounded subprocess cleanup, calculator coverage, trace retrieval coverage, and documented environment setup. The latest backend validation reports 73 passing tests and a clean Ruff run; frontend lint and production build remain part of the promotion gate.
