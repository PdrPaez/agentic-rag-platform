...
|---|---:|---:|---:|---:|---:|
| BM25 | 0.77 | 0.77 | 0.77 | 0.24 | 0.17 |
| Vector | 0.80 | 0.80 | 0.65 | 0.08 | 0.09 |
| Hybrid | 0.80 | 0.80 | 0.78 | 0.59 | 0.63 |
...
The complete first-clone validation record is in [docs/clean-clone-validation.md](docs/clean-clone-validation.md).
...
I validated the complete flow against a running backend and frontend. The validation covered model loading, demo seeding, idempotence, retrieval with citations, trace retrieval, metrics, calculator execution, restart persistence, deletion, and reseeding. The automated checks currently include 73 passing backend tests, a clean Ruff run, 93% backend coverage, deterministic retrieval and answer-behavior checks, conflict diagnostics, OpenAPI route checks, trace secret-redaction checks, provider malformed-response checks, upload path-safety checks, and successful frontend lint and production build.

The current bundled answer-behavior benchmark reports 0.80 abstention accuracy, 0.80 citation-presence accuracy, and 1.00 expected-fact coverage. These results are limited to the committed corpus and are intentionally reported as benchmark evidence rather than a general accuracy claim.
...
Retrieved documents are treated as untrusted evidence, tools are allowlisted, calculator evaluation is restricted, and
uploads and trace metadata are validated or redacted. See the complete threat model, mitigations, and production
