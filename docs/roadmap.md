...
The repository is currently synchronized through `ARAG-166`. Recent validated hardening includes deterministic ranking tie-breaking, independent benchmark latency measurement, structured retrieval metadata, recursive trace-secret redaction, cross-platform upload path sanitization, controlled provider and demo-seed failures, refreshed evaluation artifacts, provider error correlation, complete runtime configuration documentation, and a robust real HTTP smoke workflow with dynamic port allocation, bounded subprocess cleanup, calculator coverage, and trace retrieval coverage. The repository now also includes the retrieval design and technical decision log documents. The existing GitHub Actions workflow covers backend, evaluation, frontend, dependency, and smoke-test quality gates without redundant pipelines, including an enforced Ruff formatting check. A complete offline-friendly `.env.example` documents the centralized runtime configuration, including the exact supported provider values, and the changelog records the recent architectural changes. The README now links directly to the real CI workflow status. The latest backend validation reports 73 passing tests and a clean Ruff run; frontend lint and production build remain part of the promotion gate.
