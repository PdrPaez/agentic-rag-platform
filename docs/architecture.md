...
The evaluation command (`python -m app.evaluation.run`) uses a controlled five-document corpus and 30 cases, including a 20% negative slice for abstention behavior. It computes BM25, vector-only, hybrid, and hybrid-plus-reranking Hit Rate@1/3/5, Recall@5, MRR, deterministic expected-fact coverage, and average/p50/p95 latency. Fact coverage is normalized text matching for this controlled corpus; it is not semantic factuality evaluation. Backend tests also exercise deterministic answer-behavior checks for abstention, citation presence, and expected-fact coverage.
